...
async def ascii(ctx, text: str):
    if text is None:
        ...
        try:
            ...
            logger.error(f'It seems like something went wrong {e}')
            await ctx.send(embed=embed)
        finally:
...
@bot.command(name='invite', description='get the bot invite link')
...
# help command
# -------------------------------------
@bot.command(name="bothelp", description="know about the bot")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        embed = discord.Embed(
            title=':(',
            description=f"Sorry, I don't recognize that command. Please use my valid command e.g. `_slant Hello` or use `_bothelp` for help",
            color=discord.Color.red()
        )
        await ctx.send(embed=embed)
    else:
        ...
        embed = discord.Embed(
            title='Oops!',
            description=f"It seems like something went wrong while processing your command. {e} ",
            color=discord.Color.red()
        )
        await ctx.send(embed=embed)
